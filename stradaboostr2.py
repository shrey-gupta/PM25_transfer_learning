...
# from sklearn.tree import RandomForestRegressor
# from sklearn.tree import MLPRegressor
#
...
class TwoStageTrAdaBoostR2:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.sample_size is None:
            raise ValueError("Additional input required: sample size of source and target is missing. Helooo!!")
        elif np.array(self.sample_size).sum() != X.shape[0]:
            raise ValueError("Input error: the specified sample size does not equal to the input size. Heloooo!!")
        ...
        print("Inside the original two-stage TrAdaBoost.R2")
    ...
    def _twostage_adaboostR2(self, istep, X, y, sample_weight):
        ...
        if not istep == self.steps - 1:
            ...
            sample_weight[-self.sample_size[-1]:] = (sample_weight[-self.sample_size[-1]:]* np.power(
            beta2,
            1 - error_vect[-self.sample_size[-1]:]))* (self.learning_rate/sample_weight.sum())
