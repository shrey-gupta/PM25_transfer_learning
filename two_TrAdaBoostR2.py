...
class TwoStageTrAdaBoostR2:
    ...
    def fit(self, X, y, sample_weight = None):
        # Check parameters: If learning rate is non-negative
        if self.learning_rate <= 0:
            raise ValueError("learning_rate must be greater than zero")

        # Check parameters: If sample weights are not available
        if sample_weight is None:
            # Initialize weights to 1 / n_samples
            sample_weight = np.empty(X.shape[0], dtype = np.float64) #Checking condition
            sample_weight[:] = 1. / X.shape[0] # Initialize sample weights
        else: # If sample weights are available
            # Normalize existing weights: This means you can initialize sample weights
            sample_weight = sample_weight / sample_weight.sum(dtype = np.float64)
        ...
        for istep in range(self.steps):
            model = AdaBoostRegressor(self.base_estimator, n_estimators = self.n_estimators)
            # model = GradientBoostingRegressor(self.base_estimator, n_estimators = self.n_estimators)
            ...
            ## Original Equation
            sample_weight[:-self.sample_size[-1]] = (sample_weight[:-self.sample_size[-1]]* np.power(beta2,error_vect[:-self.sample_size[-1]]))* (self.learning_rate/sample_weight.sum())
            ...
            ###################
            ## Find: If ==> {X - m}*{sample_weight[:-self.sample_size[-1]]} [This is source instances multiplied by sample weights]
            ##                        >= mean(sample_weight[:-self.sample_size[-1]]) from prev. iterations (They also took values such as 0.08, 0.07, 0.06 .. so on)
            ## weights are not multiplied, otherwise they are multiplied by the similarity score.

            ### X_check = X.copy()
            ### X_check_source = X_check[:-self.sample_size[-1]]*sample_weight[:-self.sample_size[-1]
            ### for element in X_check_source:
            ###        if element >= mean:
